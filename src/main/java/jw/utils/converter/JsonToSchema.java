...
public class JsonToSchema {
    ...
    private String getJsonSchema(JsonNode jsonNode, String indent, boolean pretty) throws IOException {
    ...
    public synchronized Schema convert2AvroSchema(String json) throws IOException {
        Map<String, Object> metrics = mapper.readValue(json, LinkedHashMap.class);

        nameMapping = new HashMap<String, String>();
        Schema schema = parse2Schema(metrics);

        return schema;
    }

    public synchronized GenericRecord convert2GenericRecord(String json) throws IOException {
        Map<String, Object> metrics = mapper.readValue(json, LinkedHashMap.class);
    ...
    private Schema parse2Schema(Map<String, Object> metrics) {
        Schema schema = Schema.createRecord("Metrics", null, "juergenwewer", false);
...
//        fields.add(new Schema.Field("logLine", LogLine.getClassSchema(), null, null));
    ...
    public Schema parseRecordField(String name, Object value) {
        ...
        if (value instanceof Integer) {
            ...
            Schema record = Schema.createRecord(lookupRecordName, null, "juergenwewer", false);
            List<Schema.Field> fields = new ArrayList<Schema.Field>();
            ...
            for (Map.Entry<String, Object> e : jsonField.entrySet()) {
                ...
                if (valueSchema == null ){
                    Schema.Field field = new Schema.Field(lookupName,
                            Schema.createUnion(Arrays.asList(Schema.create(Schema.Type.NULL), parseRecordField(e.getKey(), ""))),
                            null,
                            Schema.parseJson("null"));
                    fields.add(field);
                } else {
